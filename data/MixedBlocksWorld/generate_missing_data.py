    ...
    def getDescription(self):
        ...
        if len(self.shapes) == 1:
            ...
            for i in xrange(1, len(self.shapes)):
                ...
                if self.shapes[i].__class__ == self.relations[i-1].first.__class__:
                ...
                missing.append("Ensure that %s."%self.relations[i-1].description)
        return self.description
...
class Col(Shape):
    ...
    def __init__(self, height):
        ...
        missing.append(random.choice(Col.COMPOSITIONS)%height)
    def getLowerLeftDescription(self):
...
class Square(Shape):
    def __init__(self, dim):
        ...
        if False:
			...
			self.size_description = "a square"
        description = random.choice(DESCRIPTIONS)
        #                   name, width, height, description
        Shape.__init__(self, "square", dim, dim, self.size_description)
...
class Rect(Shape):
    def __init__(self, width, height):
        ...
        if False:
			...
			self.size_description = "a rectangle"
...
while len(shapes) < TRAIN_SIZE + TEST_SIZE:
    ...
    '''
    if ''.join(composite.getDescription()) not in descriptions:
